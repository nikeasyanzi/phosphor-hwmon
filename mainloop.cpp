    ...
    SensorSet::container_t::const_reference sensor)
{
    ...
    catch (const std::system_error& e)
    {
        ...
        lg2::info("Sensor not added to dbus for read fail, file:{FILE}, return code={RC}",
                                 "FILE", file.c_str(),
                                 "RC", e.code().value());
...
void MainLoop::run()
{
    ...
    {
    lg2::error("Error in sysfs polling loop {ERROR_MSG}",
                        "ERROR_MSG", e.what());
        throw;
    }
}
    ...
    for (auto& [sensorSetKey, sensorStateTuple] : _state)
    {
        ...
        catch (const std::system_error& e)
        {
            ...
            if (sAdjusts.rmRCs.count(e.code().value()) > 0)
            {
                ...
                {
                    // Trace for sensor not already removed from dbus
         lg2::info("Remove sensor from dbus for read fail, file: {FILE}, return code:{RC}",
                                     "FILE", file.c_str(),
                                     "RC", e.code().value());
